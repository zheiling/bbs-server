...
static FORCE_INLINE int32_t rotl32 ( int32_t x, int8_t r )
{
  return (x << r) | (x >> (32 - r));
}
...
static FORCE_INLINE int32_t fmix32 ( int32_t h )
{
  h ^= h >> 16;
  h *= 0x85ebca6b;
  h ^= h >> 13;
                          ...
                          int32_t seed, void * out )
{
  ...
  const int32_t * blocks = (const int32_t *)(data + nblocks*4);

  for(i = -nblocks; i; i++)
  {
    int32_t k1 = getblock(blocks,i);
                           ...
                           int32_t seed, void * out )
{
  ...
  int32_t c1 = 0x239b961b; 
  int32_t c2 = 0xab0e9789;
  int32_t c3 = 0x38b34ae5; 
  int32_t c4 = 0xa1e38b93;

  //----------
  // body

  const int32_t * blocks = (const int32_t *)(data + nblocks*16);

  for(i = -nblocks; i; i++)
  {
    int32_t k1 = getblock(blocks,i*4+0);
    int32_t k2 = getblock(blocks,i*4+1);
    int32_t k3 = getblock(blocks,i*4+2);
    int32_t k4 = getblock(blocks,i*4+3);
